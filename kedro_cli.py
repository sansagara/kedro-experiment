...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/kedro_experiment")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=kedro_experiment"]
    )
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "kedro_experiment"
